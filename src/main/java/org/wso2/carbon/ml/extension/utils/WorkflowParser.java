...
public class WorkflowParser {
	 ...
	 */
	public MLRWorkflow parseWorkflow(String fileURL)
			throws InitializationException, FormattingException {
	...
	private MLRWorkflow populateWorkflowBean(JsonElement jsonElement) {
		...
		mlRWorkflow
				.setAlgorithmClass(workflow.getAsJsonObject().get("algorithmClass").getAsString());
	...
	private List<MLFeature> populateFeatures(JsonArray features) {
		...
		@SuppressWarnings("unchecked") Iterator<JsonElement> iterator = features.iterator();
	...
	private Map<String, String> populateParameters(JsonElement jsonElement) {
		...
		@SuppressWarnings("unchecked") Set<Map.Entry<String, JsonElement>> keys =
				parameters.entrySet();
